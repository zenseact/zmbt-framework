...
struct ExpressionGrammar : boost::spirit::karma::grammar<OutputIterator, Expression()> {
    ExpressionGrammar() : ExpressionGrammar::base_type(start)
    {
        ...
        start
            ...
            | eps(is_infix_pipe) << pipe[_1 = parameter_list]
        ...
        subexpr
            = eps(is_literal)    << karma::lazy(serialize)
            | eps(is_preproc)    << karma::lazy(serialize)
            | eps(is_infix_pipe) << nested_pipe[_1 = parameter_list]
    ...
    boost::spirit::karma::rule<OutputIterator, std::list<Expression>()> fork, pipe, parameters, nested_pipe, nested_fork;
};
